...
class DefaultTest extends TestCase
{
  ...
  {
      exec('cat '.$xmlPath .' | php '. __DIR__ .'/../cpd2pr '.$options.' 2>&1', $output, $exit);
      $output = implode("\n", $output);
  ...
  public function testFileWithMinimalWarnings() {
    $this->assertXML(__DIR__.'/errors/minimal.xml', 0, trim(file_get_contents(__DIR__.'/errors/minimal.expect')));
  }

  public function testFileWithMixedWarnings() {
    $this->assertXML(__DIR__.'/errors/mixed.xml', 0, trim(file_get_contents(__DIR__.'/errors/mixed.expect')));
  }
